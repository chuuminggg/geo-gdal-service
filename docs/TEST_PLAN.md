...
### 8.2 성능 (10 VU, 각 20초, 총 61,780 요청, 실패 0%)

| API | avg | p50 | p95 | p99 | 임계값 |
|---|---|---|---|---|---|
| route_astar | 14.5 | 12.1 | 31.8 | 47.9 | ✅ |
| route_dijkstra | 10.4 | 8.8 | 21.8 | 35.0 | ✅ |
| route_walk | 19.8 | 14.6 | 49.1 | 86.1 | ✅ |
| poi_nearby | 19.6 | 15.6 | 42.3 | 89.5 | ✅ |
| **poi_nearest** | 187.8 | 143.9 | **447.5** | 547.7 | ❌ 100ms |
| elevation | 57.6 | 32.1 | 155.3 | 202.0 | ❌ 150ms |
| pixel_value | 62.7 | 37.0 | 167.1 | 207.8 | ❌ 150ms |
| elevation_batch(100점) | 71.8 | 54.7 | 177.1 | 224.8 | ✅ |
...
| 5 | 성능 | POI KNN p95 447ms. `geom::geography <->` 인덱스 KNN 이 단건 16.6ms (geometry `<->` 는 1.7ms) - 인덱스 탐색 중 회전타원체 거리 계산 비용이 커서 동시 요청 시 DB CPU 포화 | **미조치** - geometry KNN 으로 후보 k×N 개를 먼저 뽑고 geography 거리로 재정렬 |
| 6 | 성능 | 고도/픽셀 조회 p95 155~167ms. 요청마다 `/vsis3/` 로 COG 를 열어(HEAD/헤더 읽기) 오버헤드 발생 | **미조치** - 활성 래스터별 열린 Dataset 풀(스레드별) 재사용 |
| 7 | 성능 | 경로 API 지연(p50 12ms)의 대부분이 탐색(1~2ms)이 아닌 요청당 오버헤드(활성 버전 DB 조회, 응답 직렬화) | **미조치** - 활성 버전 조회 캐시(배포 이벤트로 무효화) |

## 10. 다음 단계
- 미조치 5~7 개선 후 같은 스크립트로 재측정해 before/after 비교
